...
test.describe('Navigation is responsive (no stuck views)', () => {
  ...
  test('unified search finds a CVE and deep-links to a filtered KEV page', async ({ page }) => {
    ...
    await page.keyboard.press('Enter')
    await expect(page).toHaveURL(/[?&]q=CVE-2021-44228/)
    await expect(page.getByRole('heading', { name: 'KEV Dashboard' })).toBeVisible()
    await expect(page.getByPlaceholder(/Search vulnerabilities/)).toHaveValue('CVE-2021-44228')
  })
  ...
  test('unified search finds a threat actor', async ({ page }) => {
    ...
    await page.keyboard.press('Enter')
    await expect(page).toHaveURL(/actors[?&]q=Lazarus/)
