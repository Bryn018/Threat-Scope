...
export default function IocLookup() {
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-6 sm:px-6">
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-fg">IOC Lookup</h1>
      ...
      <form onSubmit={handleLookup} className="mb-6">
        <div className="flex flex-col gap-3 sm:flex-row">
          <div className="relative flex-1">
            <IocIcon className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted" />
            <input
              value={inputValue}
              onChange={(e) => { setInputValue(e.target.value); setError('') }}
              placeholder="Enter IP address, domain, or file hash (MD5/SHA1/SHA256)"
              className="w-full rounded-xl border border-border bg-surface-2/60 px-9 py-2.5 text-sm text-fg placeholder:text-muted focus:border-accent focus:outline-none font-mono"
            />
            {iocType && iocType !== 'unknown' && (
              <span className="absolute right-3 top-1/2 -translate-y-1/2 rounded-full border border-border-strong px-2 py-0.5 text-[10px] text-muted">
                {getIocLabel(iocType)}
              </span>
            )}
          </div>
          <button
            ...
            className="rounded-xl bg-accent px-6 py-2.5 text-sm font-medium text-accent-fg hover:opacity-90 disabled:opacity-40"
          >
      ...
      {/* Results */}
      {results && (
        <div className="mb-6 rounded-xl border border-border bg-surface-2/60 p-5">
          <div className="flex items-center gap-3 mb-4">
            <div className="rounded-lg bg-accent-soft p-2">
              {(() => { const Icon = getIocIcon(results.type); return <Icon className="h-5 w-5 text-accent" /> })()}
            </div>
            <div>
              <p className="font-mono text-sm font-bold text-fg">{results.value}</p>
              <p className="text-xs text-muted">{getIocLabel(results.type)} • {results.timestamp.toLocaleTimeString()}</p>
            </div>
          </div>

          <div className="space-y-2">
            <p className="text-xs uppercase tracking-wider text-muted mb-3">External Analysis Links</p>
              ...
              .map(([key, url]) => {
                ...
                return (
                  <a
                    ...
                    className="flex items-center justify-between rounded-lg border border-border-strong bg-surface-2/40 px-4 py-3 text-sm text-fg transition hover:border-sky-500/50 hover:text-fg"
                  >
            ...
            {results.abuseIpDb && (
              <div className="rounded-lg border border-amber-500/30 bg-amber-500/5 px-4 py-3 text-xs text-warning">
      ...
      {/* Info */}
      <div className="mb-6 rounded-xl border border-border bg-surface-2/40 p-4">
        <div className="flex items-start gap-3">
          <Info className="h-5 w-5 shrink-0 text-accent" />
          <div>
            <p className="text-sm font-medium text-fg">How IOC Lookup Works</p>
      ...
      {history.length > 0 && (
        <div>
          <h2 className="mb-3 text-sm font-semibold text-muted">Recent Lookups</h2>
          <div className="space-y-2">
            {history.map((item, idx) => (
              <button
                key={idx}
                onClick={() => { setInputValue(item.value); setResults(item) }}
                className="flex w-full items-center justify-between rounded-lg border border-border bg-surface-2/40 px-4 py-2.5 text-left transition hover:border-border-strong"
              >
                <div className="flex items-center gap-3">
                  {(() => { const Icon = getIocIcon(item.type); return <Icon className="h-4 w-4 text-muted" /> })()}
                  <span className="font-mono text-sm text-muted">{item.value}</span>
                  <span className="text-xs text-muted">{getIocLabel(item.type)}</span>
                </div>
                <span className="text-xs text-muted">{item.timestamp.toLocaleTimeString()}</span>
              </button>
            ))}
          </div>
        </div>
      ...
      {!results && !isLoading && history.length === 0 && (
        <div className="rounded-xl border border-border bg-surface-2/60 p-12 text-center">
          ...
          <p className="mt-3 text-sm text-muted">Enter an IOC to investigate</p>
          <p className="mt-1 text-xs text-muted">Supports IPs, domains, MD5, SHA-1, and SHA-256 hashes</p>
        </div>
      )}
    </div>
  )
