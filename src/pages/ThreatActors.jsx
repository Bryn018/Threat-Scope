...
function StatPill({ icon: Icon, label, value, tone = 'text-slate-300' }) {
  return (
  ...
  return (
    <button
      ...
      className={`group flex w-full flex-col gap-2 rounded-2xl border p-4 text-left transition
        ${active ? 'border-sky-500/70 bg-sky-500/5' : 'border-slate-800 bg-slate-900/40 hover:border-slate-700 hover:bg-slate-900/70'}`}
    >
    ...
    <div className="space-y-6">
      <header className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        ...
        <div className="flex flex-wrap gap-2">
          <StatPill icon={Users} label="Tracked actors" value={actors.length} tone="text-sky-300" />
          <StatPill icon={Crosshair} label="w/ techniques" value={actors.filter(a => a.techniques.length).length} tone="text-indigo-300" />
          <StatPill icon={Skull} label="w/ malware" value={actors.filter(a => a.malware.length).length} tone="text-red-300" />
        </div>
      </header>

      {isLoading && <div className="rounded-2xl border border-slate-800 bg-slate-900/40 p-8 text-center text-slate-400">Loading adversaries…</div>}
      ...
      {!isLoading && !error && (
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
          ...
          <select
            ...
            className="rounded-xl border border-slate-800 bg-slate-900/60 px-3 py-2 text-sm text-slate-200 focus:border-sky-500 focus:outline-none"
          >
        ...
        <div className="rounded-2xl border border-sky-500/30 bg-slate-900/60 p-5">
          ...
          {selected.url && (
            <a href={selected.url} target="_blank" rel="noopener noreferrer"
               className="mb-4 inline-flex items-center gap-1 text-xs text-sky-400 hover:underline">
              MITRE ATT&CK profile <ExternalLink className="h-3 w-3" />
            </a>
          )}
          {selected.description && (
            <p className="mb-4 max-w-3xl text-sm leading-relaxed text-slate-300">{selected.description}</p>
          )}
          <div className="grid gap-4 md:grid-cols-3">
            <Section icon={Crosshair} title="Techniques" tone="text-indigo-300" items={selected.techniques} empty="No mapped techniques" href={(t) => `https://attack.mitre.org/techniques/${t}/`} />
            <Section icon={Bug} title="Malware" tone="text-red-300" items={selected.malware} empty="No known malware" />
            <Section icon={Wrench} title="Tools" tone="text-emerald-300" items={selected.tools} empty="No known tools" />
          </div>
        </div>
      )}
      ...
      {!isLoading && !error && (
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
          ...
          {filtered.length === 0 && (
            <div className="col-span-full rounded-2xl border border-slate-800 bg-slate-900/40 p-8 text-center text-slate-400">No actors match “{query}”.</div>
          )}
        </div>
      )}
            ...
            <li key={it}>
              {href ? (
                <a href={href(it)} target="_blank" rel="noopener noreferrer"
                   className="inline-flex items-center gap-1 rounded-md bg-slate-800/60 px-2 py-0.5 text-xs text-slate-300 hover:bg-slate-800 hover:text-white">
                  {it}
                </a>
              ) : (
                <span className="inline-block rounded-md bg-slate-800/60 px-2 py-0.5 text-xs text-slate-300">{it}</span>
              )}
            </li>
          ))}
        </ul>
